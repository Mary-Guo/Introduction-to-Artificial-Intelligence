...
class GreedyBustersAgent(BustersAgent):
    ...
    def chooseAction(self, gameState):
        ...
        for key in livingGhostPositionDistributions:
            possible_positions.append(key.argMax())
        distance = float("inf")
        ghostpostion = possible_positions[0]
        for p in possible_positions:
            mazeDistance = self.distancer.getDistance(p, pacmanPosition)
            if mazeDistance < distance:
                distance = mazeDistance
                ghostpostion = p
        action = legal[0]
        temp = self.distancer.getDistance(Actions.getSuccessor(pacmanPosition, action), ghostpostion)
        for a in legal:
            successorPosition = Actions.getSuccessor(pacmanPosition, a)
            new_distance = self.distancer.getDistance(successorPosition, ghostpostion)
