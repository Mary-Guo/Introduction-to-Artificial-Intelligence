...
class PacmanDeepQAgent(PacmanQAgent):
    ...
    def compute_q_targets(self, minibatch, network = None, target_network=None, doubleQ=False):
        ...
        Q_predict = network.run(states).data # a numpy array of Q network's prediction of size (batch_size x num_actions) on states.
        Q_target = np.copy(Q_predict) # a numpy array of Q target of size  (batch_size x num_actions)
        # states_np (batchsize, state_size). state[0] = x, state[1] = y
        bonus = 1 / (2 * np.sqrt(self.counts[states_np[:, 0], states_np[:, 1]]/100))

        next_states_q_pred = network.run(next_states).data

        next_states_q_target = target_network.run(next_states).data

        best_action = np.argmax(next_states_q_pred, 1)

        indexes = np.arange(states_np.shape[0])
        Q_target[indexes, actions] = rewards + (1-dones)*self.discount* next_states_q_target[indexes, best_action] + bonus
